...
def tocfl_detect_train_serialize(file_name):
...
def tocfl_detect_test_serialize(file_name):
...
def tocfl_identification_train_serialize(file_name):
    logging.info('Loading train data from %s' % (file_name))
...
def tocfl_identification_test_serialize(file_name):
    logging.info('Loading test data from %s' % (file_name))
...
def tocfl_position_train_serialize(file_name, bucket_size=7, position='right'):
...
def tocfl_position_train_sample(file_name, bucket_size=7, position='right'):
    logging.info('Loading train data from %s, bucket_size: %d, position mode: %s' % (file_name, bucket_size, position))
...
def tocfl_position_test_serialize(file_name, bucket_size=7, position='right'):
    ...
    for line in my_file.readlines():
        ...
        text_line.extend(line[1].decode('utf-8'))
        # print(text_line)
...
def tocfl_final_serialize(file_name):
    logging.info('Loading test data from %s' % (file_name))
...
if __name__ == '__main__':
    ...
    train_text, R_label, M_label, S_label, W_label = tocfl_identification_train_serialize(train_file)
    print(len(train_text), len(R_label))

    test_file = os.path.join('data', 'CGED16_TOCFL_Test_Input.txt')
    test_sid, test_text = tocfl_identification_test_serialize(test_file)
    print(len(test_sid), len(test_text))

    bucket_size = 10

    train_file = os.path.join('data', 'CGED16_TOCFL_Train_All.txt')
    tocfl_position_train_sample(train_file, bucket_size=bucket_size, position='right')
